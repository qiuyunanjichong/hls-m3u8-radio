...
public class Uploader implements InitializingBean {
	...
	private ExecutorService executorService = new BlockingThreadPoolExecutor(1,
			10, new ArrayBlockingQueue<Runnable>(10));
		...
		new Thread(new Runnable() {
			...
			public void run() {
				StringBuffer url = new StringBuffer().append("http://")
						...
						.append("/m3u8-center/upload.do?tsdir=");
				while (true) {
					try {
...
//							File newf = 
//							tmpf.renameTo( )
							url.append(Constant.BB_ts_Dir).append("&tsname=")
	...
	public void upload(final String url, final File f) {
		executorService.submit(new Runnable() {
			...
			public void run() {
				// System.out.println(f);
				try {
					log.info("upload {}", httpUpload.upload(url, f));
				} catch (Exception e) {
